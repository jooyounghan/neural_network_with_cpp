...
int main()
{
	while (true)
	{
		CNNModel model = CNNModel(1, 1, 2, 5, 32, 64, 128, 64, 32);
		model.SetActivationFunc(6, AF_IDENTITY, AF_RELU, AF_RELU, AF_RELU, AF_RELU, AF_RELU);
		...
		for (uint32 iter = 0; iter < 1000; ++iter)
		{
			...
			std::shared_ptr<CMatrix> a6 = std::make_shared<CMatrix>(1, 1, new double{ 6 });
			std::shared_ptr<CMatrix> b6 = std::make_shared<CMatrix>(1, 2, new double[] { 1, 0 });
			model.PushInput(a6, b6, 0.01);

			std::shared_ptr<CMatrix> a7 = std::make_shared<CMatrix>(1, 1, new double{ 7 });
		...
		std::shared_ptr<CMatrix> a8 = std::make_shared<CMatrix>(1, 1, new double{ 3 });
		std::shared_ptr<CMatrix> result8 = model.GetResult(a8);
		std::cout << "result of 8 : ";
		result8.get()->PrintData();
		std::shared_ptr<CMatrix> a9 = std::make_shared<CMatrix>(1, 1, new double{ 4 });
		std::shared_ptr<CMatrix> result9 = model.GetResult(a9);
		std::cout << "result of 9 : ";
